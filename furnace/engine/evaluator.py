...
class Evaluator(object):
    def __init__(self, dataset, class_num, image_mean, image_std, network,
                 ...
                 verbose=False, save_path=None, save_image=False):
        self.eval_time = 0
    ...
    def scale_process(self, img, ori_shape, crop_size, stride_rate,
        ...
        if long_size <= min(crop_size[0], crop_size[1]):
            ...
            #score = self.val_func_process(input_data, device)[0]
            ...
            for grid_yidx in range(r_grid):
                for grid_xidx in range(c_grid):
                    ...
                    #temp_score = self.val_func_process(input_data, device)[0]
            ...
            # print(score)
            # sys.exit(0)
            #os.system(f'cat scoree {score.size()} > p.txt')
            score = score[:, margin[0]:(score.shape[1] - margin[1]),
                    margin[2]:(score.shape[2] - margin[3])]
        ...
        #print(score.shape)
        # sys.exit(0)
        #score = score.view(score.shape[-2], score.shape[-1], score.shape[0])
