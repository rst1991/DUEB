...
with Engine(custom_parser=parser) as engine:
    ...
    loss_al = bayesian_categorical_crossentropy(10,19)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            unsup_imgs_0 = unsup_minibatch_0['data'].to(device)
            unsup_imgs_1 = unsup_minibatch_1['data'].to(device)
            mask_params = unsup_minibatch_0['mask_params'].to(device)
            ...
            "Supervised Part"
            #print(imgs.shape)
            _, sup_pred_1, sup_varp_1 = model(imgs, step=1)
            _, sup_pred_2, sup_varp_2 = model(imgs, step=2)
            loss_sup_1 = criterion_sup(sup_pred_1, gts)
            loss_sup_2 = criterion_sup(sup_pred_2, gts)
            soft1 = F.softmax(sup_pred_1, dim=1)
            soft2 = F.softmax(sup_pred_2, dim=1)
            ls1 = get_values(soft1, gts) #get_values selects logits from pred according to ground truth class
            ls2 = get_values(soft2, gts)

            # print('var',sup_varp_1.size())
            loss_all1 = loss_al(sup_pred_1, sup_varp_1, gts)
            loss_all2 = loss_al(sup_pred_2, sup_varp_2, gts)
            loss_all = 0.5*loss_all1 + 0.5*loss_all2
            predlog_sup1= torch.logsumexp(ls1, 0)
            predlog_sup2= torch.logsumexp(ls2, 0)
            loss_sup = loss_sup_1 + loss_sup_2 + loss_all + 0.5*predlog_sup1 + 0.5*predlog_sup2 

            "Unsupervised Part"
            # Estimate the pseudo-label
            with torch.no_grad():
                # teacher#1
                _, logits_u0_tea_1, varp_u0_tea_1 = model(unsup_imgs_0, step=1)
                _, logits_u1_tea_1, varp_u1_tea_1 = model(unsup_imgs_1, step=1)
                logits_u0_tea_1 = logits_u0_tea_1.detach()
                logits_u1_tea_1 = logits_u1_tea_1.detach()
                varp_u0_tea_1 = varp_u0_tea_1.detach()
                varp_u1_tea_1 = varp_u1_tea_1.detach()

                # print("logits",logits_u0_tea_1.size())
                # print("rinit",varp_u0_tea_1.size())
                # teacher#2
                _, logits_u0_tea_2, varp_u0_tea_2 = model(unsup_imgs_0, step=2)
                _, logits_u1_tea_2, varp_u1_tea_2 = model(unsup_imgs_1, step=2)
                logits_u0_tea_2 = logits_u0_tea_2.detach()
                logits_u1_tea_2 = logits_u1_tea_2.detach()
                varp_u0_tea_2= varp_u0_tea_2.detach()
                varp_u1_tea_2= varp_u1_tea_2.detach()


            # Mix teacher predictions using same mask
            logits_cons_tea_1 = logits_u0_tea_1 * (1 - batch_mix_masks) + logits_u1_tea_1 * batch_mix_masks
            #print('lmix',logits_cons_tea_1.size())
            _, ps_label_1 = torch.max(logits_cons_tea_1, dim=1)
            #print('ps',ps_label_1.size())
            ...
            # student#1
            _, logits_cons_stu_1, varp_cons_stu_1  = model(unsup_imgs_mixed, step=1)
            # student#2
            _, logits_cons_stu_2, varp_cons_stu_2  = model(unsup_imgs_mixed, step=2)
            ...
            loss_unsupt = (loss_unsup_inter + loss_unsup_union) * config.unsup_weight 
            
            loss_all_unsup1 = loss_al(logits_cons_stu_1, varp_cons_stu_1, ps_label_inter)
            loss_all_unsup2 = loss_al(logits_cons_stu_2, varp_cons_stu_2, ps_label_union)
            loss_all_unsup= loss_all_unsup1 + loss_all_unsup2
            #print(loss_all_unsup)

            
            soft3= F.softmax(logits_cons_stu_1, dim=1)
            soft4= F.softmax(logits_cons_stu_2, dim=1)
            us1 = get_values(soft3, ps_label_inter)
            us2 = get_values(soft4, ps_label_union)
            predlog_unsup1=torch.logsumexp(us1, 0)
            predlog_unsup2=torch.logsumexp(us2, 0)
            loss_unsup=loss_unsupt + loss_all_unsup +predlog_unsup1 + predlog_unsup2 
            ...
            print_str = 'Epoch{}/{}'.format(epoch, config.nepochs) \
                        ...
                        + ' (inter=%.2f' % loss_unsup_inter.item() \
                        + ' union=%.2f  ' % loss_unsup_union.item() \
                        + ' predlog_unsup1=%.2f' % predlog_unsup1.item() \
                        + ' predlog_unsup2=%.2f)' % predlog_unsup2.item() \
                        + ' loss_sup=%.2f' % loss_sup.item() \
                        + ' (sup_1=%.2f' % loss_sup_1.item() \
                        + ' sup_2=%.2f  ' % loss_sup_2.item() \
                        + ' overlap=%.2f' % overlap \
                        + ' predlog_sup1=%.2f)' % predlog_sup1.item() \
                        + ' predlog_sup2=%.2f)' % predlog_sup2.item() \
                        + ' loss_all=%.2f)' % loss_all.item() \
                        + ' loss_all_unsup=%.2f)' % loss_all_unsup.item() \
