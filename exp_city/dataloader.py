...
def get_train_loader(engine, dataset, train_source, unsupervised=False, collate_fn=None):
    ...
    if unsupervised is False:
        train_dataset = dataset(data_setting, "train", train_preprocess, config.max_samples, unsupervised=False)
    else:
        train_dataset = dataset(data_setting, "train", train_preprocess, config.max_samples, unsupervised=True)
...
class CityScape(BaseDataset):
    ...
    def __init__(self, setting, split_name, preprocess=None, file_length=None, training=True, unsupervised=False):
