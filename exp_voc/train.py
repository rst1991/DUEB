...
with Engine(custom_parser=parser) as engine:
    ...
    loss_al = bayesian_categorical_crossentropy(10,21)
    ...
    for epoch in range(engine.state.epoch, config.nepochs):
        ...
        for idx in pbar:
            ...
            #loss_sup = loss_sup_1 + loss_sup_2+ loss_all
            predlog_sup1= torch.logsumexp(ls1, 0)
            predlog_sup2= torch.logsumexp(ls2, 0)
        
            pred_sup  = 0.5*predlog_sup1 + 0.5*predlog_sup2
            loss_sup = loss_sup_1 + loss_sup_2 + 0.5*predlog_sup1 + 0.5*predlog_sup2 + loss_all
            
            #print("hhbhb", loss_sup)
            ...
            #print(torch.unique(ps_label_inter))
            ...
            loss_unsup=loss_unsupt + predlog_unsup1 + predlog_unsup2 + loss_all_unsup
            #print('sumun', loss_unsup)
            ...
            #           + ' predlogu1=%.2f)' % predlog_unsup1.item() \
            #            + ' predlogu2=%.2f)' % predlog_unsup2.item() \
            #            + ' predlog_sup1=%.2f)' % pred_sup.item() \
            #            + ' overlap=%.2f' % overlap
