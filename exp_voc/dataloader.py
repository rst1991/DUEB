...
def get_train_loader(engine, dataset, train_source, unsupervised=False, collate_fn=None):
    ...
    if 'selftrain' in train_source:
        ...
        train_dataset = dataset(data_setting, "train", train_preprocess, config.max_samples, unsupervised=unsupervised)
...
class VOC(BaseDataset):
    def __init__(self, setting, split_name, preprocess=None, file_length=None, training=True, unsupervised=False, pseudo_label=False):
    ...
    def __getitem__(self, index):
        ...
        if gt is not None:
            gt = np.uint8(gt)
            #print(torch.unique(torch.tensor(gt)))
        ...
        output_dict = dict(data=img, fn=str(names),n=len(self._file_names))
        if gt is not None:
            extra_dict['label'] = gt
